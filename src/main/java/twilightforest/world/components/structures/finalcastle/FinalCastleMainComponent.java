...
public class FinalCastleMainComponent extends TFStructureComponentOld {
	...
	public void addChildren(StructurePiece parent, StructurePieceAccessor list, Random rand) {
		...
		buildTowerMaze(list, rand, 48, 0, 24, 60, Direction.SOUTH, TFBlocks.PINK_CASTLE_RUNE_BRICK.defaultBlockState(), dest);
	...
	private boolean isMazeComplete(StructurePieceAccessor list, BlockState type) {
		if (list instanceof StructureStart<?> start) {
			...
			for (StructurePiece structurecomponent : start.getPieces()) {
				...
				if (type == TFBlocks.PINK_CASTLE_RUNE_BRICK.defaultBlockState() && structurecomponent instanceof FinalCastleEntranceTowerComponent) {
					return true;
				}
	...
	public boolean postProcess(WorldGenLevel world, StructureFeatureManager manager, ChunkGenerator generator, Random rand, BoundingBox sbb, ChunkPos chunkPosIn, BlockPos blockPos) {
		...
		// door, second floor
		this.generateBox(world, sbb, 0, 31, 23, 0, 34, 25, TFBlocks.VIOLET_CASTLE_DOOR.defaultBlockState(), AIR, false);
